...
from ..xiuxian_utils.utils import (
    check_user, get_msg_pic, number_to, markdown
)
...
async def fusion_help_(bot: Bot, event: GroupMessageEvent):
    ...
    buttons = [
        [(2, '合成', '合成', False), (2, '查看可合成物品', '查看可合成物品', True)],            
    ]
...
async def fusion_(bot: Bot, event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    sql_message.update_ls(user_id, required_stone, 2)
    for item_id, amount_needed in needed_items.items():
        sql_message.update_back_j(user_id, int(item_id), amount_needed)

    sql_message.send_back(user_id, int(equipment_id), equipment['name'], equipment['type'], 1, 0)

    item_type = equipment.get('type', '物品')
    msg = f"道友成功合成了{item_type}: {equipment['name']}！！"
    params_items = [('msg', msg)]               
    buttons = [
        [(2, '查看修仙物品', f'查看修仙物品{args_str}', True)],            
    ]
    data = await markdown(params_items, buttons)
    await bot.send_group_msg(group_id=int(send_group_id), message=MessageSegment("markdown", {"data": data})) 
    await fusion.finish()
