...
from ..xiuxian_utils.utils import check_user, get_msg_pic, number_to, markdown, post_image_to_server, encode_image_to_base64
...
async def xiuxian_message_(bot: Bot, event: GroupMessageEvent):
    ...
    if re.search(r"图片版", command_text):
        img_res = await draw_user_info_img(user_id, DETAIL_MAP)
      #  base64_image = encode_image_to_base64(img_res)
      #  image_info = post_image_to_server(base64_image, GROUP_ID, MSGID, API_URL)
      #  img_path = image_info['url']
       # params_items = [('msg', f'![修仙信息](http://multimedia.nt.qq.com/{img_path})')]               
       # buttons = [
