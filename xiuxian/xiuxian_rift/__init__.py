...
from ..xiuxian_utils.utils import (
    check_user, check_user_type, markdown,
...
async def rift_help_(bot: Bot, event: GroupMessageEvent, session_id: int = CommandObjectID()):
    ...
    if session_id in cache_help:
        ...
        buttons = [
            [(2, '探索秘境', '探索秘境', True), (2, '秘境结算', '秘境结算', True)],            
            [(2, '探索终止', '秘境探索终止', True)],
        ]
       # 调用 markdown 函数生成数据
...
async def create_rift_(bot: Bot, event: GroupMessageEvent):
   ...
   #         await bot.send_group_msg(group_id=int(send_group_id), message=msg)
    ...
    try:
        ...
        msg = f"当前已存在{group_rift[group_id].name}，秘境可探索次数：{group_rift[group_id].count}次，请诸位道友发送 探索秘境 去探索吧！"
        params_items = [('msg', msg)]               
        buttons = [
            [(2, '探索秘境', '探索秘境', True)],            
           # [(2, '探索终止', '秘境探索终止 ', True)],
       ...
       # 调用 markdown 函数生成数据
        ...
        rift.time = config['rift'][rift.name]['time']
        group_rift[group_id] = rift
        msg = f"野生的{rift.name}出现了！秘境可探索次数：{rift.count}次，请诸位道友发送 探索秘境 去探索吧！"
        params_items = [('msg', msg)]               
        buttons = [
            [(2, '探索秘境', '探索秘境', True)],            
           # [(2, '探索终止', '秘境探索终止 ', True)],
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if not is_type:
      ...
      #      await explore_rift.finish()
        ...
        if user_vip_days > 0:
            msg = f"道友进入秘境：{group_rift[group_id].name}，由于你是月卡用户，探索需要花费时间：{group_rift[group_id].time // 2}分钟！"
        else:        
