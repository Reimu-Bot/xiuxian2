...
from .utils import (get_msg_pic, markdown_s)
...
def Cooldown(
    ...
    async def dependency(bot: Bot, matcher: Matcher, event: MessageEvent):
        ...
        if group_id in conf_data["group"]:
            if event.sender.role == "admin" or event.sender.role == "owner" or True:
                bot = await assign_bot_group(group_id=group_id)
                
                # 定义消息和按钮
                msg = "本群修仙模组已关闭，是否需要启用修仙功能？"
                ...
                # 发送按钮消息
                await bot.send_group_msg(group_id=int(group_id), message=MessageSegment("markdown", {"data": data}))
